...
rows34 = run_query("select DENSE_RANK() OVER (PARTITION BY DATE ORDER BY sum(SP_F) DESC) AS RANK, Agent, sum(sp_f), DATE from SCOREBOARD_MAR2023 where type='FDN' and Team='Team 9' group by Agent, Date;")
df34=pd.DataFrame(rows34)
df34.columns += 1
df34.columns = ["Rank","Agent Name", "Funded", "Date"]
df34['Funded'] = df34['Funded'].astype(int)

rows35 = run_query("select DENSE_RANK() OVER (PARTITION BY DATE ORDER BY sum(SP_F) DESC) AS RANK, Agent, sum(sp_f), DATE from SCOREBOARD_MAR2023 where type='EFS' and Team='Team 9' group by Agent, Date;")
df35=pd.DataFrame(rows35)
df35.columns += 1
df35.columns = ["Rank","Agent Name", "Funded", "Date"]
df35['Funded'] = df35['Funded'].astype(int)

rows36 = run_query("select DENSE_RANK() OVER (PARTITION BY DATE ORDER BY sum(SP_F) DESC) AS RANK, Agent, sum(sp_f), DATE from SCOREBOARD_MAR2023 where type='EFS' and Team='Team 10' group by Agent, Date;")
...
df38.columns += 1
df38.columns = ["Rank","Agent Name", "Funded", "Date"]
df38['Funded'] = df38['Funded'].astype(int)

#rows39 = run_query("select DENSE_RANK() OVER (PARTITION BY DATE ORDER BY sum(SP_F) DESC) AS RANK, Agent, sum(sp_f), DATE from SCOREBOARD_MAR2023 where type='EFS' and Team='Team 12' group by Agent, Date;")
#df39=pd.DataFrame(rows39)
#df39.columns += 1
...
if selected_option == "EFS":
   ...
   if radio == 'Team 1':
      ...
      with col2:
           st.subheader('Top Team 9 Agents')
           st.table(filtered_df_35[["Rank","Agent Name", "Funded"]].head(10))     
     ...
     # with col1:
      ...
      #with col2:
          # st.subheader('Top Team 12 Agents')
          # st.table(filtered_df_39[["Rank","Agent Name", "Funded"]].head(10)) 
   ...
   if radio == 'Team 1':
       ...
       with col4:
        st.subheader('Top Team 11 Agents')
        st.table(filtered_df_38[["Rank","Agent Name", "Funded"]].head(10))
     ...
     #  with col4:
     #   st.subheader('Top Team 12 Agents')
     #   st.table(filtered_df_40[["Rank","Agent Name", "Funded"]].head(10))
